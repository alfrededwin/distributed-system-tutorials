...
import ds.tutorials.sycnhronization.tx.DistributedTx;
import ds.tutorials.sycnhronization.tx.DistributedTxCoordinator;
import ds.tutorials.sycnhronization.tx.DistributedTxListener;
import ds.tutorials.sycnhronization.tx.DistributedTxParticipant;
...
//    private static final java.util.UUID UUID = ;
    ...
    @Override
    public void setBalance(ds.tutorial.communication.grpc.generated.SetBalanceRequest request, io.grpc.stub.StreamObserver<ds.tutorial.communication.grpc.generated.SetBalanceResponse> responseObserver) {
        ...
        if (server.isLeader()){
            ...
            try {
                ...
                startDistributedTx(accountId, value); updateSecondaryServers(accountId, value); System.out.println("going to perform"); if (value > 0){
    ...
    private void updateBalance() {
        if (tempDataHolder != null) {
            ...
            server.setAccountBalance(accountId, value); System.out.println("Account " + accountId + " updated to value " + value + " committed");
            tempDataHolder = null;
        }
    }
    ...
    private SetBalanceResponse callServer(String accountId, double value, boolean isSentByPrimary, String IPAddress, int port) {
        ...
        channel = ManagedChannelBuilder.forAddress(IPAddress, port)
                ...
                .build();
        clientStub =
                SetBalanceServiceGrpc.newBlockingStub(channel);
        SetBalanceRequest request = SetBalanceRequest .newBuilder()
    ...
    private SetBalanceResponse callPrimary(String
                                                   accountId, double value) {
        System.out.println("Calling Primary server");
        String[] currentLeaderData =
                server.getCurrentLeaderData();
        String IPAddress = currentLeaderData[0];
        int port =
                Integer.parseInt(currentLeaderData[1]);
        return callServer(accountId, value, false,
                IPAddress, port);
    }
    private void updateSecondaryServers(String accountId,
                                        double value) throws KeeperException,
            InterruptedException {
        System.out.println("Updating secondary servers");
        List<String[]> othersData =
                server.getOthersData();
        for (String[] data : othersData) {
            String IPAddress = data[0];
            int port = Integer.parseInt(data[1]); callServer(accountId, value, true, IPAddress, port);
        }
    }
