...
public class DistributedTxParticipant extends DistributedTx implements Watcher {
    ...
    public void voteCommit()  {
        try {
            if (currentTransaction != null) { System.out.println("Voting to commit the transaction " + currentTransaction);
    ...
    public void voteAbort() {
        try {
            if (currentTransaction != null) { System.out.println("Voting to abort the transaction " + currentTransaction);
    ...
    void onStartTransaction(String transactionId, String
        ...
        try {
            ...
            currentTransaction = transactionRoot +
                    PARTICIPANT_PREFIX + participantId; client.createNode(currentTransaction, true,
                    CreateMode.EPHEMERAL, "".getBytes(StandardCharsets.UTF_8));
